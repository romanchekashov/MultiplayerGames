...
function M.find_opponent_and_join_match(match_callback)
...
function M.send_player_move(data)
	if not match then
		return
	end

	nakama.sync(function()
		log("Sending match_data message " .. data)
		local result = realtime.match_data_send(socket, match.match_id, OP_CODE_MOVE, data)
		if result.error then
			log(result.error.message)
			pprint(result)
		end
...
local handle_match_data
-- handle received match data
-- decode it and pass it on to the game
function M.on_handle_match_data(callback)
	handle_match_data = callback
end
...
function M.login(callback)
	...
	nakama.sync(function()
		...
		if not ok then
			...
			callback(nil, "Unable to create socket connection")
			return
		end
		...
		-- 	M.send_player_move(match.match_id, row, col)
		-- end)
